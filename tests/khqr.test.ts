...
describe("generateKHQR", () => {
  ...
  it("uses the PayWay KHQR frame layout", async () => {
    ...
    const svg = atob(result.replace("data:image/svg+xml;base64,", ""));
    expect(svg).toContain('viewBox="0 0 218 316"');
    expect(svg).toContain('width="218" height="316"');
    expect(svg).toContain('aria-label="PayWay KHQR payment card"');
    ...
    expect(svg).toContain('id="qr-area" x="37" y="145" width="144" height="144"');
    expect(svg).toContain('aria-label="KHQR brand mark"');
    expect(svg).not.toContain('id="aba-pay-logo"');
    expect(svg).not.toContain(">ABA<");
    expect(svg).not.toContain(">PAY<");
    expect(svg).not.toContain("Scan with ABA Mobile");
    expect(svg).not.toContain("supported banking app");
